server <- function(session, input, output) {
  ...
  save_data <- function(pruned_data, extraction_done = 0) {
    ...
    update_db(conn, "source_evidence_predictive_enrichment", "source_name", "sources")
    ...
    if (isFALSE(ascertain_required_fields(pruned_data()))) {
      showModal(
        modalDialog(
          ...
          "If this is a bug, hit Save and throw BSKH an email with the RCT name.",
          easyClose = TRUE,
          label = "Got it!"
        )
      )
  ...
  observeEvent(input$publication_year_principal_report, {
    year_regex <- "0000|19[0-9][0-9]|20[0-1][0-9]|202[0-3]"
  ...
  observeEvent(input$recruitment_period, {
    period_regex <- "^([0-9]|1[0-2])/(0000|19[0-9][0-9]|20[0-1][0-9]|202[0-3])-([0-9]|1[0-2])/(0000|19[0-9][0-9]|20[0-1][0-9]|202[0-3])$"
  ...
  output$rct_selector <- renderUI({
    ...
    if (length(choices) == 0) { 
      ...
      choices <- if (EXTRACTOR() == "BSKH") {
        dbGetQuery(conn, glue("SELECT rct_name FROM rcts WHERE pending_bskh = 1"))$rct_name
      } else if (EXTRACTOR() == "FINAL") {
        dbGetQuery(conn, glue("
          SELECT rct_name FROM rcts
          EXCEPT
          SELECT rct_name FROM extractions WHERE extractor = 'FINAL' AND extraction_done = 1
        "))$rct_name
      } else {
        dbGetQuery(conn, glue("
          SELECT rct_name FROM rcts WHERE pending_rest = 1
          EXCEPT
          SELECT rct_name FROM extractions WHERE extractor NOT IN ('initial', 'FINAL', 'BSKH')")) %>%
  ...
  output$the_form <- renderUI({
    ...
    sources <- fetch_choices(conn, "source_name", "sources")
    ...
    tagList(
      ...
      fluidRow(
        box(width = 6,
          ...
          selectizeInput("source_evidence_predictive_enrichment", "Source of evidence for predictive enrichment", width = "100%", choices = c("", sources), selected = defaults()$source_evidence_predictive_enrichment %||% character(0), options = list(create = TRUE)),
  ...
  output$progress_plot_workhorse <- renderPlot({
    conn <- make_conn()
    df <- dbGetQuery(conn, "
      SELECT extractor, rct_name, extracted_data::json ->> 'timestamp' AS timestamp 
      FROM extractions
    ...
    ") %>%
      ...
      distinct(extractor, rct_name, .keep_all = TRUE) %>% # handle when more than one of the others have extracted on the same
      group_by(extractor) %>%
      mutate(
        n_extracted = row_number(timestamp),
        extractor = sprintf("%s (%i)", extractor, max(n_extracted))
      ) 
    ...
    p <- ggplot(bind_rows(df, pad_df), aes(timestamp, n_extracted, colour = extractor, group = extractor)) +
      ...
      geom_hline(yintercept = 321, linetype = 2, size = 0.5) +
      annotate(geom = "label", x = ymd_hms("2022-09-14 12:00:00 UTC"), y = 321, label = "Target", hjust = 0) +
    ...
    if (EXTRACTOR() == "BSKH") {
      p + 
        scale_y_continuous(trans = scales::pseudo_log_trans(sigma = 20, base = 10)) +
        geom_hline(yintercept = 10, linetype = 2, size = 0.25) +
        geom_hline(yintercept = 20, linetype = 2, size = 0.25)
    } else {
      p
    }
  })
